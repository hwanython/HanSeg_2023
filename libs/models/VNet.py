...
class VNetLight(BaseModel):
    ...
    def __init__(self, elu=True, in_channels=1, n_classes=4):
        ...
        self.up_tr64 = UpTransition(128, 64, 1, elu)
        self.up_tr32 = UpTransition(64, 32, 1, elu)
        self.out_tr = OutputTransition(32, n_classes, elu)
...
class VNetLight2(BaseModel):
    """
    A lighter version of Vnet that skips down_tr256 and up_tr256 in oreder to reduce time and space complexity
    """

    def __init__(self, elu=True, in_channels=1, classes=4):
        super(VNetLight2, self).__init__()
        self.classes = classes
        self.in_channels = in_channels
        ...
        self.up_tr128 = UpTransition(64, 64, 2, elu, dropout=True)
    ...
    def test(self, device='cpu'):
        input_tensor = torch.rand(1, self.in_channels, 180, 96, 96)
        ideal_out = torch.rand(1, self.classes, 180, 96, 96)
        out = self.forward(input_tensor)
        assert ideal_out.shape == out.shape
        print("Vnet light test is complete")
