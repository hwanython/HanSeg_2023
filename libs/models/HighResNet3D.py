...
class Conv1x1x1(nn.Module):
    def __init__(self, in_channels, classes):
        ...
        bn1 = torch.nn.BatchNorm3d(in_channels)
        relu1 = nn.ReLU()
...
class HighResNet3D(BaseModel):
    ...
    def test(self):
        k = 32
        x = torch.rand(1, self.in_channels, 256, k, k)
        pred = self.forward(x)
        target = torch.rand(1, self.classes, 256, k, k)
...
def test_all_modules():
    ...
    a = torch.rand(1, 32, 256, k, k)
    m3 = DilatedConv2(in_channels=32)
    y = m3(a)
    assert y.shape == a.shape, print(y.shape)
    print("DilatedConv2 OK")

    a = torch.rand(1, 48, 256, k, k)
    m4 = DilatedConv4(in_channels=48)
    y = m4(a)
    assert y.shape == a.shape, print(y.shape)
    print("DilatedConv4 OK")

    m4 = Conv1x1x1(in_channels=48, classes=33)
    y = m4(a)
    print(y.shape)
...
if __name__ == '__main__':
    test_all_modules()
    model = HighResNet3D(in_channels=1, classes=33)
    model.test()