...
if __name__ == '__main__':
    ...
    if config.trainer.do_train:
        ...
        for epoch in range(experiment.epoch, config.trainer.epochs+1):
            ...
            if epoch % 3 == 0:
                test_dice = experiment.test(phase="Test")
        ...
        logging.info(f'''
                Best test Dice found: {best_test['value']} at epoch: {best_test['epoch']}
                ''')
    ...
    if config.trainer.do_test:
        ...
        experiment.load()
        test_dice = experiment.test(phase="Test")
        logging.info(f'Test results Dice: {test_dice}')
